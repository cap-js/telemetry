...
const zti = require('./zti')
const { getDiagLogLevel, hasDependency, _require } = require('./utils/common')
const { getResource } = require('./utils/resource')
const { setupStaticCerts } = require('./utils/mtls')

// Provision the client certificate for mTLS exporters once at startup: prefer ZTI (which also
// starts watching for rotations), else fall back to statically configured x509. ZTI is an
// asynchronous source — SVID files may land shortly after startup — whereas static x509 is
// present now or never. Record which kind we got so exporter wiring knows whether a missing
// cert is expected-and-transient (async) or a hard misconfiguration. Only relevant for CaaS.
function _provisionCerts() {
  if (!/to-caas$/.test(cds.env.requires.telemetry.kind || '')) return
  const asyncCerts = zti.setup()
  if (!asyncCerts) setupStaticCerts()
  cds.env.requires.telemetry._asyncCerts = asyncCerts
}
...
module.exports = () => {
  // provision mTLS certs before building exporters
  _provisionCerts()

  if (hasDependency('@sap/xotel-agent-ext-js')) setup_with_calm()
  else setup_standalone()
}
