...
// starts watching for rotations), else fall back to statically configured x509. The resolved
// source is recorded so exporter wiring knows whether a missing cert is expected (ZTI, arriving
// async) or a misconfiguration (static, never arriving). Only relevant for the CaaS kind.
function _provisionCerts() {
  if (!/to-caas$/.test(cds.env.requires.telemetry.kind || '')) return
  const useZTI = zti.setup()
  if (!useZTI) setupStaticCerts()
  cds.env.requires.telemetry._useZTI = useZTI
}
