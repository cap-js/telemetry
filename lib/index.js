...
function _getSampler() {
  ...
  const samplers = require('@opentelemetry/sdk-trace-base')
  if (kind === 'ParentBasedSampler') return new samplers[kind]({ root: new samplers[root](ratio) })
  return new samplers[kind]()
}
...
function instrumentApplication() {
  ...
  // build instrumentations from config
  const instrumentations = []
  for (const each of Object.values(cds.env.requires.telemetry.instrumentations)) {
    const Instrumentation = require(each.package)[each.class]
    instrumentations.push(new Instrumentation({ ...each.config }))
  }
  ...
  const tracerProvider = new NodeTracerProvider({
    resource,
    sampler: _filterSampler(_ignoreSpecifiedPaths, _getSampler())
  })
