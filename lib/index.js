...
/**
 * Check if ZTI is needed for CaaS telemetry
 */
...
async function setup_standalone() {
  ...
  if (needsZTI) {
    // ZTI: Register TracerProvider early so HTTP instrumentation gets a real tracer
    ...
    // Create exporters after ZTI credentials are ready
    metrics(resource)
    if (cds.env.requires.telemetry.logging) logging(resource)
