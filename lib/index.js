...
function _needsZTIWait() {
  ...
  // Look for zero-trust-identity binding
  for (const services of Object.values(vcapServices)) {
...
async function setup_standalone() {
  ...
  if (needsZTI) {
    // ZTI FLOW: Register TracerProvider early, add exporter after SVID ready
    // This ensures HTTP instrumentation gets a real tracer instead of NoopTracer
    // while waiting for SVID certificates

    // STEP 1: Create TracerProvider EARLY (before ZTI wait)
    ...
    // STEP 2: Register instrumentations IMMEDIATELY
    ...
    // STEP 4: Add trace exporter now that credentials are available
