...
function _getInstrumentations() {
  ...
  for (const [key, each] of Object.entries(_instrumentations)) {
    ...
    // inject our responseHook for http/undici instrumentation to propagate attributes to parent CDS span
    if (key === 'http' || each.module === '@opentelemetry/instrumentation-http' ||
        key === 'undici' || each.module === '@opentelemetry/instrumentation-undici') {
      const { responseHook: cdsResponseHook } = require('./tracing/httpHooks')
      const userResponseHook = config.responseHook
      config.responseHook = userResponseHook
        ? (span, response) => {
            cdsResponseHook(span, response)
            userResponseHook(span, response)
          }
        : cdsResponseHook
    }
