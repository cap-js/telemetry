...
function setup_standalone() {
  ...
  // This returns a promise, but we handle async initialization gracefully:
  // - If ZTI is not configured, this resolves immediately
  // - If ZTI is configured, SVID files are loaded during tracing/metrics setup
  const ztiInit = initializeZTI().catch(err => {
