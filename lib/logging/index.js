...
module.exports = resource => {
  ...
  cds.on('served', () => {
    ...
    const custom_fields = cds.env.requires.telemetry.logging.custom_fields || []

    // intercept logs via format
    const { format } = cds.log
    cds.log.format = function (module, level, ...args) {
      const res = format.call(this, module, level, ...args)
