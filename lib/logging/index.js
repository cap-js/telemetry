...
const { getCredsForCLSAsUPS, augmentCLCreds, augmentCaaSCreds, getCredsForCaaSMtls, _require } = require('../utils')
...
function _getExporter() {
  ...
  if (kind.match(/to-caas$/)) {
    ...
    const lazyExporter = createLazyExporter(() => {
      ...
      return new loggingExporterModule[loggingExporter.class](exporterConfig)
    })
    LOG._debug && LOG.debug('Using lazy logs exporter for CaaS')
    return lazyExporter
  }
