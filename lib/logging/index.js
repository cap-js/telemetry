...
function _getLoggingConfig() {
  ...
  LOG._debug && LOG.debug('_getLoggingConfig called', { kind, logging, loggingType: typeof logging })

  // If logging is an object with exporter, use it directly
  if (logging && typeof logging === 'object' && logging.exporter) {
    LOG._debug && LOG.debug('Using logging config from telemetry.logging object')
    return logging
  }
  ...
  if (logging === true && kind) {
    ...
    LOG._debug && LOG.debug('Looking up logging config from kind', { kind, hasKindConfig: !!kindConfig, hasLoggingExporter: !!kindConfig?.logging?.exporter })
    if (kindConfig?.logging?.exporter) {
      return kindConfig.logging
    }
  }

  LOG._debug && LOG.debug('_getLoggingConfig returning null')
  return null
}
