...
const { getCredsForCLSAsUPS, augmentCLCreds } = require('../utils/credentials')
const { createCaaSExporter } = require('../utils/caas')
const { _require } = require('../utils/common')
...
// @opentelemetry/sdk-logs 0.221 changed the log processor constructors
...
function _getExporter() {
  ...
  if (kind === 'telemetry-to-caas') {
    exporter = createCaaSExporter(
      ...
      config,
      '/v1/logs',
      cfg => new loggingExporterModule[loggingExporter.class](cfg)
    )
  }

  exporter ??= new loggingExporterModule[loggingExporter.class](config)
  ...
  cds.on('served', () => {
    ...
    const format = (cds.log.format = function (module, level, ...args) {
      ...
      if (log) {
        ...
        if (log.tenant_id) attributes['sap.tenancy.tenant_id'] = log.tenant_id
