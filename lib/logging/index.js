...
const { getCredsForCLSAsUPS, augmentCLCreds, createCaaSExporter, _require } = require('../utils')
  ...
  cds.on('served', () => {
    ...
    const format = (cds.log.format = function (module, level, ...args) {
      ...
      if (log) {
        ...
        // add tenant + correlation for parity with spans and metrics
        const ctx = cds.context
        if (ctx?.tenant) attributes['sap.tenancy.tenant_id'] = ctx.tenant
        if (ctx?.id) attributes['sap.cds.correlation_id'] = ctx.id
