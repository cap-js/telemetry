  ...
  if (kind === 'telemetry-to-caas') {
    ...
    if (credentials.useZTI) {
      return createLazyExporter(() => {
        ...
        try {
          return { status: 'ok', exporter: new loggingExporterModule[loggingExporter.class](exporterConfig) }
        } catch (error) {
    ...
    const exporter = new loggingExporterModule[loggingExporter.class](exporterConfig)
