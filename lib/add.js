...
module.exports = class TelemetryTemplate extends cds.add.Plugin {
  ...
  async canRun() {
    ...
    if ((hasHelmUnifiedRuntime || hasHelm) && !hasMta)
      throw `'cds add telemetry' is not available for Kyma yet`
    return true
  }
  ...
  async combine() {
    ...
    if (hasMta) {
      ...
      if (hasMultitenancy) {
        const mtxSidecar = mtxSidecar4(
          isJava ? 'mtx/sidecar' : 'gen/mtx/sidecar'
        )
