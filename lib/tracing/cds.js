...
module.exports = () => {
  ...
  // Add request-time diagnostics by wrapping ApplicationService.prototype.handle
  cds.once('served', () => {
    if (cds.ApplicationService?.prototype?.handle) {
      const _appHandle = cds.ApplicationService.prototype.handle
      cds.ApplicationService.prototype.handle = async function handle(req) {
        const requiresResolving = this._requires_resolving?.(req)
        LOG._debug && LOG.debug('>>> ApplicationService.handle called', {
          event: req?.event,
          service: this?.name,
          requiresResolving,
          resolved: req?._resolved,
          willCallSuper: !requiresResolving
        })
        const result = await _appHandle.apply(this, arguments)
        LOG._debug && LOG.debug('>>> ApplicationService.handle completed', {
          event: req?.event,
          service: this?.name
        })
        return result
      }
      LOG._debug && LOG.debug('ApplicationService.prototype.handle wrapped for diagnostics')
    }
  })
