...
module.exports = () => {
  ...
  // Create wrapper functions with explicit markers
  const emitWrapper = function emit() {
    LOG._debug && LOG.debug('>>> Service.prototype.emit WRAPPER called', { event: arguments[0]?.event || arguments[0], service: this?.name })
    const event = arguments[0]?.event || arguments[0]
    return trace({ phase: 'emit', event }, _emit, this, arguments, {})
  }
  emitWrapper.__telemetry_wrapper = true

  const handleWrapper = function handle(req) {
    LOG._debug && LOG.debug('>>> Service.prototype.handle WRAPPER called', { event: req?.event, service: this?.name })
    if (!_tx && req.event in _TX) return _handle.apply(this, arguments)
    return trace(req, _handle, this, arguments, {})
  }
  handleWrapper.__telemetry_wrapper = true

  cds.Service.prototype.emit = wrap(_emit, { wrapper: emitWrapper })
  cds.Service.prototype.handle = wrap(_handle, { wrapper: handleWrapper })

  LOG._debug && LOG.debug('CDS wrapping installed', {
    handleIsWrapper: cds.Service.prototype.handle.__telemetry_wrapper === true,
    handleIsWrapped: cds.Service.prototype.handle.__wrapped === true
  })

  // Wrap ApplicationService.prototype.handle which shadows Service.prototype.handle
  cds.once('served', () => {
    if (cds.ApplicationService?.prototype?.handle && !cds.ApplicationService.prototype.handle.__telemetry_wrapper) {
      const _appHandle = cds.ApplicationService.prototype.handle
      const appHandleWrapper = function handle(req) {
        LOG._debug && LOG.debug('>>> ApplicationService.prototype.handle WRAPPER called', { event: req?.event, service: this?.name })
        if (!_tx && req.event in _TX) return _appHandle.apply(this, arguments)
        return trace(req, _appHandle, this, arguments, {})
      }
      appHandleWrapper.__telemetry_wrapper = true
      cds.ApplicationService.prototype.handle = wrap(_appHandle, { wrapper: appHandleWrapper })
      LOG._debug && LOG.debug('ApplicationService.prototype.handle wrapped for tracing')
    }

    const currentHandle = cds.Service.prototype.handle
    LOG._debug && LOG.debug('CDS wrapper verification at served', {
      handleIsWrapper: currentHandle.__telemetry_wrapper === true,
      handleIsWrapped: currentHandle.__wrapped === true,
      handleName: currentHandle.name,
      appServiceHandleWrapped: cds.ApplicationService?.prototype?.handle?.__telemetry_wrapper === true
    })
  })

  // Check each service instance to see if handle is bound to instance vs prototype
  cds.on('serving', service => {
    const hasOwnHandle = Object.hasOwn(service, 'handle')
    const handleSource = hasOwnHandle ? 'instance' : 'prototype'
    const protoChain = []
    let proto = Object.getPrototypeOf(service)
    while (proto && proto !== Object.prototype) {
      if (Object.hasOwn(proto, 'handle')) {
        protoChain.push(proto.constructor?.name || 'unknown')
      }
      proto = Object.getPrototypeOf(proto)
    }
    LOG._debug && LOG.debug('>>> Service instance handle diagnostics', {
      serviceName: service.name,
      hasOwnHandle,
      handleSource,
      protoChainWithHandle: protoChain,
      instanceHandleIsWrapper: service.handle?.__telemetry_wrapper === true
    })

    // If instance has own handle, wrap it too!
    if (hasOwnHandle && !service.handle.__telemetry_wrapper) {
      const _instanceHandle = service.handle.bind(service)
      service.handle = function handle(req) {
        LOG._debug && LOG.debug('>>> Instance handle WRAPPER called', {
          event: req?.event,
          service: this?.name
        })
        return trace(req, _instanceHandle, this, arguments, {})
      }
      service.handle.__telemetry_wrapper = true
      LOG._debug && LOG.debug('>>> Wrapped instance handle for', { serviceName: service.name })
    }
  })
