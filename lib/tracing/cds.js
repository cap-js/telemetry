...
module.exports = () => {
  ...
  // Verify wrapper is still in place after CDS finishes loading
  cds.once('served', () => {
    ...
    // Also check ApplicationService
    if (cds.ApplicationService) {
      const appHandle = cds.ApplicationService.prototype.handle
      const serviceProto = Object.getPrototypeOf(cds.ApplicationService.prototype)
      LOG._debug && LOG.debug('ApplicationService diagnostics', {
        appHandleIsOwn: Object.hasOwn(cds.ApplicationService.prototype, 'handle'),
        serviceProtoHandle: serviceProto.handle.__telemetry_wrapper === true,
        superWouldCallWrapper: serviceProto === cds.Service.prototype
      })
    }
  })
