...
module.exports = () => {
  LOG._debug && LOG.debug('CDS method wrapping initialized')
  const { _tx, _hana_prom, _wrap_hana } = cds.env.requires.telemetry.tracing
  const _TX = { BEGIN: 1, COMMIT: 1, ROLLBACK: 1 }

  // Check if cds.Service exists and has prototype methods
  LOG._debug && LOG.debug('CDS wrapping diagnostics:', {
    hasService: !!cds.Service,
    hasPrototype: !!cds.Service?.prototype,
    prototypeEmit: typeof cds.Service?.prototype?.emit,
    prototypeHandle: typeof cds.Service?.prototype?.handle
  })

  if (!cds.Service?.prototype?.handle) {
    LOG._warn && LOG.warn('CDS wrapping: cds.Service.prototype.handle not available yet!')
    return
  }
  ...
  cds.Service.prototype.emit = wrap(_emit, {
    wrapper: function emit() {
      LOG._debug && LOG.debug('>>> CDS wrapped EMIT called', { event: arguments[0]?.event || arguments[0], service: this?.name })
  ...
  cds.Service.prototype.handle = wrap(_handle, {
    wrapper: function handle(req) {
      LOG._debug && LOG.debug('>>> CDS wrapped HANDLE called', { event: req?.event, service: this?.name })
  ...
  LOG._debug && LOG.debug('CDS wrapping complete:', {
    emitWrapped: cds.Service.prototype.emit.__wrapped === true,
    handleWrapped: cds.Service.prototype.handle.__wrapped === true
  })
  ...
  cds.on('serving', service => {
    // Log service details to verify wrapping works at runtime
    LOG._debug && LOG.debug('>>> CDS serving event:', {
      serviceName: service.name,
      serviceType: service.constructor?.name,
      prototypeHandleWrapped: cds.Service.prototype.handle?.__wrapped === true,
      instanceHasHandle: typeof service.handle === 'function',
      instanceHandleIsWrapped: service.handle?.__wrapped === true
    })
