...
function trace(name, fn, targetObj, args, options = {}) {
  ...
  if (targetObj.dbc || options.sql) {
    ...
    if (process.env.SAP_PASSPORT && targetObj.dbc?.constructor.name in { HDBDriver: 1, HANAClientDriver: 1 }) {
      ...
      LOG._debug &&
        LOG.debug(`select * from M_MERGED_TRACES where PASSPORT_TRANSACTION_ID = '${traceId.toUpperCase()}';`)

      // // workaround for FIXME above (see also "this.dbc._passport" in index.js)
      // targetObj.dbc._passport ??= passport
    }
