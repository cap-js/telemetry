...
function trace(req, fn, that, args, opts = {}) {
  ...
  if (name.match(/^@cap-js\/\w+ - \w+$/) && options.attributes[ATTR_DB_QUERY_TEXT]) {
    ...
    const operation = SQL_VERB[options.attributes[ATTR_DB_OPERATION_NAME]] ?? options.attributes[ATTR_DB_OPERATION_NAME]
    const table = options.attributes[ATTR_DB_SQL_TABLE]
    if (operation || table) name += ` ${[operation, table].filter(Boolean).join(' ')}`
  }
