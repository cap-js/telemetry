...
function _getDynamicDBAttributes(options, args, parentSpan) {
  ...
  if (db_statement) dbAttributes.set(SemanticAttributes.DB_STATEMENT, db_statement)
  const db_operation = args[0].event || parentSpan?.attributes[SemanticAttributes.DB_OPERATION]
  if (db_operation) dbAttributes.set(SemanticAttributes.DB_OPERATION, db_operation)
  const db_sql_table = args[0].target?.name || parentSpan?.attributes[SemanticAttributes.DB_SQL_TABLE]
  if (db_sql_table) dbAttributes.set(SemanticAttributes.DB_SQL_TABLE, db_sql_table)
  return dbAttributes
}
...
function trace(name, fn, targetObj, args, options = {}) {
  ...
  if (targetObj.dbc || options.sql) {
    ...
    if (
      ...
      parentSpan?.attributes[SemanticAttributes.DB_SYSTEM] &&
      !parentSpan.attributes[SemanticAttributes.DB_STATEMENT]
    ) {
      parentSpan.setAttribute(SemanticAttributes.DB_STATEMENT, span.attributes[SemanticAttributes.DB_STATEMENT])
    }
  }
