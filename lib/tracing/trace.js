...
if (HRTIME) cds.on('listening', ({ server }) => server.on('request', req => (req[$hrnow] = _hrnow())))
...
function trace(name, fn, that, args, options = {}) {
  // REVISIT: only start tracing once served
  if (!cds._telemetry.tracer._active) return fn.apply(that, args)
  ...
  const ctx = isAsync
    ...
    : otel.context.active()
  const spanOptions = {
    kind: _determineKind(that, name?.phase, isAsync, options)
  }
  ...
  let spanName = typeof name === 'string' ? name : _getSpanName(name, fn, that)
