...
// > Possibly replaced by ATTR_CODE_COLUMN_NUMBER from stable_attributes?
...
function _getStaticAttributes(fn, options) {
  if (!fn[$reqattrs]) {
    ...
    if (fn.name) {
      attributes.set(ATTR_CODE_FUNCTION, fn.name) // REVISIT: Replaced by ATTR_CODE_FUNCTION_NAME?
      attributes.set(ATTR_CODE_FUNCTION_NAME, fn.name)
    } else if (options?.fn) {
      // REVISIT: case _hana_prom = true
      attributes.set(ATTR_CODE_FUNCTION, options?.fn) // REVISIT: Replaced by ATTR_CODE_FUNCTION_NAME?
    ...
    if (fn.__location) {
      ...
      attributes.set(ATTR_CODE_FILEPATH, path) // REVISIT: Replaced by ATTR_CODE_FILE_PATH?
      ...
      attributes.set(ATTR_CODE_LINENO, location.line) // REVISIT: Replaced by ATTR_CODE_LINE_NUMBER?
