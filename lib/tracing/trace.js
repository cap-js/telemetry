...
const HRTIME = cds.env.requires.telemetry.tracing._hrtime && cds.env.requires.telemetry.tracing._hrtime !== 'false'
const now = Date.now()
...
if (HRTIME) cds.on('listening', ({ server }) => server.on('request', req => (req.__hrtime = _hrnow())))
...
function _getParentSpan() {
  ...
  if (!cds.context._otelctx) {
    ...
    if (HRTIME && parent && !parent.__adjusted) {
      parent.startTime = cds.context.http?.req?.__hrtime || _hrnow()
