...
function _getSpanName(arg, fn, that) {
  ...
  if (fn.name && that.constructor.name !== 'OData') {
    if (phase) return `${that.name}.${fn.name} - ${phase} - ${event}${path ? ` ${path}` : ''}`
    if (that.name) return `${that.name} - ${event}${path ? ` ${path}` : ''}`
...
function _determineKind(that, phase, isAsyncConsumer, options) {
  // DB Calls & Remote calls are client calls
  if (that.dbc || that.constructor.name === 'RemoteService' || options.outbound) return SpanKind.CLIENT
  // cds.spawn or srv.emit
  if (that.constructor.name === 'cds' || phase === 'emit') return SpanKind.PRODUCER
...
function trace(req, fn, that, args, opts = {}) {
   ...
   * create span
   */
  const parent = _getParentSpan()
  const isAsync = parent?._is_async && !parent?.name.match(/cds\.spawn/)
  const ctx = isAsync
    ? ROOT_CONTEXT
    : parent
    ? otel.trace.setSpan(otel.context.active(), parent)
    : otel.context.active()
  const spanOptions = {
    kind: _determineKind(that, req?.phase, isAsync, opts)
  }
  // needed for sampling decision (cf. shouldSample)
  if (cds.context?.http?.req) {
    const url_path = cds.context.http.req.baseUrl + cds.context.http.req.path
  ...
  if (isAsync) {
    spanOptions.links = [{ context: parent.spanContext() }]
    spanOptions.parent = undefined
  }
  let spanName = typeof req === 'string' ? req : _getSpanName(req, fn, that)
  if (spanName.length > 80 && _truncate_span_name !== false) spanName = spanName.substring(0, 79) + '…'
  const span = tracer.startSpan(spanName, spanOptions, ctx)
  if (req.event?.match(/^cds\.spawn/) || req?.phase === 'emit') span._is_async = true
   ...
   */
  _addAttributes(span, fn, that, opts, args, parent)

  /*
   * call original function and subsequently end trace in callback
   */
  return otel.context.with(cds.context?._otelctx.setValue(cds.context._otelKey, span), async () => {
    try {
