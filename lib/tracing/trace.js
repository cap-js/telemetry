...
//   if (that instanceof cds.MessagingService) return SpanKind.CONSUMER
...
function trace(req, fn, that, args, opts = {}) {
  ...
  // note: SpanKind.PRODUCER always passed in opts
  let kind = opts.kind
  if (kind == null) {
    if (that instanceof cds.RemoteService) kind = SpanKind.CLIENT
    else if (that instanceof cds.MessagingService) kind = SpanKind.CONSUMER
    else kind = SpanKind.INTERNAL
  }
