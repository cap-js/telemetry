...
function trace(req, fn, that, args, opts = {}) {
  ...
  if (parent?.instrumentationScope?.name === '@opentelemetry/instrumentation-http' && !parent[$adjusted]) {
    ...
    // REVISIT: mutating `parent.name` (and `parent.startTime` above) post-start crosses
    //          the line of OTel's conceptual ReadableSpan immutability — the SDK tolerates it today, but
    //          a future @opentelemetry/sdk-trace-base release could enforce immutability and break this
    //          silently. Cleaner would be to build a wrapper span or set these on span creation instead.
