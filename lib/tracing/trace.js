...
function _getStaticDBAttributesMT(settings, tenant) {  
  let attributes = attrCache.get(tenant)
  if (!attributes) {    
    const attributes = new Map()
    attributes.set(SemanticAttributes.DB_SYSTEM, cds.db.options.kind) // hanadb, postgresql, sqlite
    attributes.set(SemanticAttributes.DB_NAME, cds.db.vcap?.name || cds.db.name)
    attributes.set(SemanticAttributes.DB_USER, settings.user)
    attributes.set(SemanticAttributes.DB_CONNECTION_STRING, settings.url)
    attributes.set(SemanticAttributes.NET_PEER_NAME, settings.host)
    attributes.set(SemanticAttributes.NET_PEER_PORT, settings.port)   
    attrCache.set(tenant, attributes)
  }
  module.exports.__dbAttributes = attrCache.get(tenant)
...
function trace(name, fn, targetObj, args, options = {}) {
  ...
  if (targetObj.dbc || options.sql) {
    if (cds.requires.multitenancy) {
      // settings for hdb and @sap/hana-client
      const settings = targetObj?.dbc?._connection?._settings || targetObj?.dbc?._creds
      const tenant = targetObj?.dbc?._tenant
      _setAttributes(span, settings && tenant ? _getStaticDBAttributesMT(settings, tenant) :  _getStaticDBAttributes()) 
    } else {
