...
let tracer
let _hrtimePatched = false

// Lazy-initialize tracer on first use - this avoids race condition with async ZTI init
// The tracer is obtained from the global TracerProvider, which is registered by tracing/index.js
// IMPORTANT: Don't cache until a real TracerProvider is registered, otherwise we cache a NoopTracer
function _getTracer() {
  // Check if a real TracerProvider is registered (not the default NoopTracerProvider)
  // The global trace API returns a ProxyTracerProvider that delegates to the registered provider
  const registeredProvider = otel.trace.getTracerProvider()
  const delegate = registeredProvider?.getDelegate?.()
  const isNoopProvider = !registeredProvider ||
    registeredProvider.constructor.name === 'ProxyTracerProvider' &&
    delegate?.constructor.name === 'NoopTracerProvider'

  LOG._debug && LOG.debug('_getTracer() check:', {
    providerName: registeredProvider?.constructor?.name,
    delegateName: delegate?.constructor?.name,
    isNoopProvider,
    tracerCached: !!tracer
  })

  if (isNoopProvider) {
    // TracerProvider not yet registered - don't cache, return null to skip tracing
    return null
  }

  if (!tracer) {
    tracer = otel.trace.getTracer('@cap-js/telemetry', require('../../package.json').version)
    LOG._debug && LOG.debug('_getTracer() created new tracer')

    // Apply hrtime patches once when tracer is first obtained
    if (HRTIME && !_hrtimePatched) {
      _hrtimePatched = true
...
// Register listening handler for hrtime - this still needs 'served' but it's just for the optional hrtime feature
if (HRTIME) {
  cds.on('served', () => {
...
function trace(req, fn, that, args, opts = {}) {
  // only trace if there is a cds.context
  if (!cds.context) {
    LOG._debug && LOG.debug('trace() skipped: no cds.context')
    return fn.apply(that, args)
  }

  // Lazy-init tracer on first use (avoids race with async ZTI init)
  const currentTracer = _getTracer()
  if (!currentTracer) {
    LOG._debug && LOG.debug('trace() skipped: no tracer (NoopProvider still active)')
    return fn.apply(that, args)
  }
  ...
  return currentTracer.startActiveSpan(name, options, span => {
