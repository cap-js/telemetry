...
function _getParentSpan() {
  ...
  const r1 = otel.context.active().getValue(cds.context._otelKey)
  const r2 = cds.context._otelctx.getValue(cds.context._otelKey)
  return r1 || r2
}
...
function trace(name, fn, targetObj, args, options = {}) {
  // only start tracing once served and there is a context
  if (!cds._tracer || !cds.context) return fn.apply(targetObj, args)

  const activeSpan = otel.trace.getActiveSpan()

  // if the current request matches instrumentation-http's ignoreIncomingPaths -> abort
  if (cds.context.http?.req && !activeSpan) return fn.apply(targetObj, args)

  // root span gets request attributes
  if (cds.context.http?.req && !activeSpan.__adjusted) {
    activeSpan.__adjusted = true
    _setAttributes(activeSpan, _getRequestAttributes())
    if (HRTIME) activeSpan.startTime = cds.context.http?.req?.__hrnow || _hrnow()
    if (ADJUST_ROOT_NAME && activeSpan.attributes[SEMATTRS_HTTP_TARGET]) activeSpan.name += ' ' + activeSpan.attributes[SEMATTRS_HTTP_TARGET]
  }
  ...
  if (isAsync) {
    ...
    debugger
    ctx = otel.trace.setSpan(otel.context.active(), parentSpan)
    debugger
  } else {
    debugger
    ctx = otel.context.active()
    debugger
  }
  ...
  // if (span.constructor.name === 'NonRecordingSpan') {
  //   debugger
  ...
  if (targetObj.dbc || options.sql) {
    ...
    if (span.attributes[SEMATTRS_DB_STATEMENT] && parentSpan?.attributes[SEMATTRS_DB_SYSTEM] && !parentSpan.attributes[SEMATTRS_DB_STATEMENT]) {
