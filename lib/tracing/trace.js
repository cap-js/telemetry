...
function _getSpanName(arg, fn, targetObj) {
  ...
  if (targetObj.dbc) {
    const trgt = _getDBTarget(targetObj)
...
function _getDBAttributes(name, targetObj) {
  if (!module.exports.__dbAttributes) {
    ...
    // attributes.set(SemanticAttributes.DB_CONNECTION_STRING, cds.db.options) // TODO: clarify what the value should be
    ...
    // attributes.set(SemanticAttributes.NET_TRANSPORT, cds.db.options) // TODO: clarify what value
 ...
 * @returns
 */
module.exports = function trace(name, fn, targetObj, args, options = {}) {
  ...
  // otherAttributes.set('sap.cds.logger', options.loggerName || LOG.label)
  // REVISIT: correct?
  if (targetObj.constructor.name === 'cds' || name?.phase === 'emit') {
    // cds for cds.spawn - emit for srv.emit
    otherAttributes.set('sap.cds.async', true)
  }
  ...
  return otel.context.with(cds.context._otelctx.setValue(cds.context._otelKey, span), () => {
    let res, is_async
    try {
      res = fn.apply(targetObj, args)
      is_async = res instanceof Promise
      if (is_async) {
        res
          .then(res => {
      ...
      !is_async && span.end(HRTIME ? process.hrtime() : undefined)
    }
    return res
  })
}
