...
function _getParentSpan() {
  ...
  if (!cds.context._otelctx) {
    ...
    if (parent && !parent.__adjusted) {
      ...
      if (ADJUST_ROOT_NAME && parent.attributes[ATTR_HTTP_TARGET])
        parent.name += ' ' + parent.attributes[ATTR_HTTP_TARGET]
    }
...
function _getRequestAttributes() {
  ...
  if (!cds.context.http.req.__attributes) {
    ...
    attributes.set(ATTR_HTTP_HOST, req.headers.host) //> deprecated
    attributes.set(ATTR_SERVER_ADDRESS, req.headers.host)

    const full = req.protocol + '://' + req.headers.host + req.url
    attributes.set(ATTR_HTTP_URL, full) //> deprecated
    attributes.set(ATTR_URL_FULL, full)

    attributes.set(ATTR_HTTP_SCHEME, req.protocol) //> deprecated
    attributes.set(ATTR_URL_SCHEME, req.protocol)

    attributes.set(ATTR_HTTP_TARGET, req.url) //> deprecated
    const [path, query] = req.url.split('?')
    attributes.set(ATTR_URL_PATH, path)
    if (query) attributes.set(ATTR_URL_QUERY, query)

    attributes.set(ATTR_HTTP_METHOD, req.method) //> deprecated
...
function _getStaticDBAttributes(tenant = '', creds) {
  ...
  if (!module.exports.__dbAttributes.has(tenant)) {
    ...
    attributes.set(ATTR_NET_PEER_NAME, creds ? creds.host : cds.env.requires.db.credentials.host)
    attributes.set(ATTR_NET_PEER_PORT, creds ? creds.port : cds.env.requires.db.credentials.port)
