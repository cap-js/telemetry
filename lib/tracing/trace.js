...
function trace(name, fn, targetObj, args, options = {}) {
  ...
  // temp
  const w3c_trace_id = cds.context.http?.req.headers.traceparent?.split('-')[1]
  const w3c_parent_id = cds.context.http?.req.headers.traceparent?.split('-')[2]
  if (w3c_trace_id) {
    span.setAttribute('w3c_trace_id', w3c_trace_id)
    span.setAttribute('w3c_parent_id', w3c_parent_id)
  }
