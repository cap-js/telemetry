...
module.exports = resource => {
  ...
  // workaround for FIXME in trace.js
  if (process.env.SAP_PASSPORT) {
    cds.on('served', () => {
      cds.db?.before('*', async function (req) {
        if (this.dbc?.constructor.name in { HDBDriver: 1, HANAClientDriver: 1 }) {
          if (req.event === 'BEGIN') {
            // clear passport for new tx
            delete this.dbc._passport
            await this.dbc.exec(`UNSET SESSION 'SAP_PASSPORT';`)
          } else {
            // set passport once for tx
            if (this.dbc._passport && this.dbc._passport !== 'set') {
              LOG.debug('Setting SAP Passport:', this.dbc._passport)
              await this.dbc.exec(`SET SESSION 'SAP_PASSPORT'='${this.dbc._passport}';`)
              this.dbc._passport = 'set'
            }
          }
        }
      })
