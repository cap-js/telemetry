...
module.exports = resource => {
  ...
  if (via_one_agent) {
    // Probe (#502): does OneAgent actually register a global OTel TracerProvider we can bind to?
    // 1.6.0 reused OneAgent's concrete provider via getDelegate() and bound the auto-instrumentations
    // to it, which nested CDS spans under the PurePath. #504's return-nothing path binds them to the
    // late-binding proxy instead, and in the field the CDS spans vanish. Log what OneAgent exposes,
    // then reuse the concrete delegate as 1.6.0 did (OneAgent preloads, so it is already registered).
    const proxy = trace.getTracerProvider()
    const delegate = proxy.getDelegate?.()
    LOG.info(
      'Dynatrace OneAgent path: global provider =',
      proxy?.constructor?.name,
      '| delegate =',
      delegate?.constructor?.name ?? '<none>'
    )
    return delegate
  }
