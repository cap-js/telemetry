...
module.exports = resource => {
  ...
  if (via_one_agent) {
    ...
    // HACKS:
    // - do not export @opentelemetry/instrumentation-hdb spans if xotel agent is used
    // - remove parentSpanId from http spans if there is no traceparent header
    if (process.execArgv?.some(a => a.match(/@sap\/xotel-agent-ext-js/))) {
      ...
      exporter.export = function (spans, resultCallback) {
        ...
        for (const _span of _spans) if (_should_be_orphan(_span)) _make_orphan(_span)
...
const _make_orphan = span => {
  LOG._debug && LOG.debug('Removing parentSpanId from span:', span)
