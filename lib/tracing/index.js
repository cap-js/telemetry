 ...
 * The span processor with exporter is added later via addTracingExporter().
 */
...
function addTracingExporter(tracerProvider) {
  ...
  // OpenTelemetry v2.x removed addSpanProcessor - access internal MultiSpanProcessor array
  // This is a workaround for the ZTI timing issue where we need to add the exporter after
  // TracerProvider is already registered
  if (typeof tracerProvider.addSpanProcessor === 'function') {
    // v1.x API
    tracerProvider.addSpanProcessor(processor)
  } else if (tracerProvider._activeSpanProcessor?._spanProcessors) {
    // v2.x - push to internal array
    tracerProvider._activeSpanProcessor._spanProcessors.push(processor)
  } else {
    LOG._error && LOG.error('Unable to add span processor - unknown TracerProvider API')
    return
  }
  LOG._debug && LOG.debug('Trace exporter added to TracerProvider')
}
