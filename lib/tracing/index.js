...
const { getDynatraceMetadata } = require('../utils/resource')
const { getCredsForDTAsUPS, getCredsForCLSAsUPS, augmentCLCreds } = require('../utils/credentials')
const { createCaaSExporter } = require('../utils/caas')
const { hasDependency, _require } = require('../utils/common')
...
function _getExporter() {
  ...
  if (kind === 'telemetry-to-caas') {
    exporter = createCaaSExporter(
      credentials,
      config,
      '/v1/traces',
      cfg => new tracingExporterModule[tracingExporter.class](cfg)
    )
  }

  exporter ??= new tracingExporterModule[tracingExporter.class](config)
...
module.exports = resource => {
  ...
  // rely on OneAgent's in-process OpenTelemetry capture instead of exporting the spans ourselves.
  const via_one_agent =
  ...
  if (via_one_agent) {
    // In CALM, @sap/xotel-agent-ext-js owns the provider — don't register a competing one.
    if (!resource) return
    // Register a real, recording provider with NO span processor: OneAgent observes the spans
    // in-process, so exporting them ourselves would only duplicate them. The provider must still
    // exist and record — otherwise the global tracer stays a no-op and neither the CDS spans nor
    // the http instrumentation spans are ever created. An empty processor list (not an undefined
    // processor) keeps the first span from crashing while adding no export path of our own.
