...
/**
 * A span processor that buffers spans until a delegate processor is set.
 * Used for ZTI flow where we need to register TracerProvider early but
 * can't create the exporter until credentials are available.
 */
class BufferingSpanProcessor {
  constructor() {
    this._buffer = []
    this._delegate = null
    this._maxBufferSize = 2048 // Prevent unbounded memory growth
  }
  ...
  onStart(span, context) {
    ...
    // No buffering needed for onStart - spans are mutable until onEnd
  }
  ...
  onEnd(span) {
    if (this._delegate) {
      ...
      // Buffer the span until delegate is ready
      if (this._buffer.length < this._maxBufferSize) {
        this._buffer.push(span)
      } else {
        LOG._warn && LOG.warn('BufferingSpanProcessor: buffer full, dropping span')
      }
    }
  }

  /**
   * Set the delegate processor and flush buffered spans
   */
  setDelegate(processor) {
    this._delegate = processor
    // Flush buffered spans
    const buffered = this._buffer.length
    for (const span of this._buffer) {
      processor.onEnd(span)
    }
    this._buffer = []
    if (buffered > 0) {
      LOG._debug && LOG.debug(`BufferingSpanProcessor: flushed ${buffered} buffered spans`)
    }
  }
...
/**
 * Create TracerProvider early (before credentials are available).
 * This allows HTTP instrumentation to get a real tracer instead of NoopTracer.
 * Uses a BufferingSpanProcessor that holds spans until the real exporter is ready.
 * Call addTracingExporter() later to set the real exporter.
 */
function createTracerProvider(resource) {
  ...
  // Create a buffering processor that will hold spans until real exporter is ready
  ...
  // Store reference to buffering processor for later delegate assignment
  tracerProvider._bufferingProcessor = bufferingProcessor

  LOG._debug && LOG.debug('TracerProvider registered early (exporter will be added when credentials ready)')

  return tracerProvider
}

/**
 * Add exporter to TracerProvider after credentials are available (e.g., after ZTI SVID ready).
 * This must be called after createTracerProvider() and after any async credential initialization.
 */
function addTracingExporter(tracerProvider) {
  ...
  // Set the real processor as delegate on the BufferingSpanProcessor
  // This flushes any buffered spans and forwards future spans to the real processor
  if (tracerProvider._bufferingProcessor) {
    tracerProvider._bufferingProcessor.setDelegate(processor)
    LOG._debug && LOG.debug('Trace exporter added to TracerProvider via BufferingSpanProcessor')
  } else {
    LOG._error && LOG.error('Unable to add span processor - no BufferingSpanProcessor found')
  }
}

/**
 * Original combined function for backward compatibility with CALM setup.
 * Creates TracerProvider AND adds exporter in one call.
 */
...
// Export the new split functions and keep backward compatibility
module.exports = setupTracing
module.exports.createTracerProvider = createTracerProvider
module.exports.addTracingExporter = addTracingExporter
