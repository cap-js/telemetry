...
/**
 * Span processor that buffers spans until a delegate is set.
 * Used for ZTI flow where TracerProvider must be registered early.
 */
class BufferingSpanProcessor {
  constructor() {
    this._buffer = []
    this._delegate = null
    this._maxBufferSize = 2048
  }

  onStart(span, context) {
    if (this._delegate) this._delegate.onStart(span, context)
  }
  ...
  setDelegate(processor) {
    ...
    for (const span of this._buffer) processor.onEnd(span)
    this._buffer = []
  }

  forceFlush() {
    return this._delegate ? this._delegate.forceFlush() : Promise.resolve()
  }

  shutdown() {
    this._buffer = []
    return this._delegate ? this._delegate.shutdown() : Promise.resolve()
  }
}
...
/**
 * Create TracerProvider early (for ZTI flow).
 * Uses BufferingSpanProcessor until real exporter is ready.
 */
...
/**
 * Add exporter to TracerProvider (for ZTI flow, after credentials ready).
 */
function addTracingExporter(tracerProvider) {
  ...
  const processor = _createSpanProcessor(exporter)
...
/**
 * Standard setup - creates TracerProvider with exporter in one call.
 */
