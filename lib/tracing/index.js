...
module.exports = resource => {
  ...
  /*
   * when Dynatrace OneAgent is active, route CDS spans into its already-registered global
   * TracerProvider instead of building our own (as @cap-js/telemetry did up to 1.6.0)
   */
  const via_one_agent =
  ...
  if (via_one_agent) {
    // OneAgent has registered the global TracerProvider and exports spans itself, so no
    // exporter is needed. Our CDS spans obtain their tracer via the global OpenTelemetry API
    // (see ./trace.js), so they flow into OneAgent's provider. Do NOT build/register our own
    // provider (it would clobber OneAgent's global) and do NOT create a span processor (there
    // is no exporter) — registering an undefined processor would crash on the first span in
    // MultiSpanProcessor.onStart. See #502.
    //
    // Return nothing: the only consumer of this return value is registerInstrumentations()
    // in ../index.js, which falls back to `trace.getTracerProvider()` — the global proxy —
    // when the argument is falsy. Returning the proxy is deliberately preferred over the
    // unwrapped `.getDelegate()`: the proxy is late-binding (a delegate set after tracers are
    // handed out is picked up retroactively), whereas getDelegate() snapshots the current
    // delegate — which would pin the auto-instrumentations to the Noop provider forever if
    // OneAgent's provider weren't registered yet. Letting the fallback supply the live proxy
    // is both safer and less code.
