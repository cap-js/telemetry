...
module.exports = resource => {
  ...
  if (via_one_agent) {
    // Probe (#502): OneAgent registers NO in-process OTel provider (proven: global delegate is
    // NoopTracerProvider), so returning nothing leaves getTracer() global on the noop and CDS
    // spans are never created. Instead register our OWN provider with NO exporter/processor, as
    // @cap-js/telemetry did up to 1.6.0 — OneAgent, having preloaded, is expected to capture
    // spans from the registered SDK provider. An empty processor list is valid in 2.x; only an
    // undefined processor crashes MultiSpanProcessor.onStart.
    ...
    const delegate = trace.getTracerProvider().getDelegate?.()
    LOG.info('OneAgent probe: after register, global delegate =', delegate?.constructor?.name ?? '<none>')
