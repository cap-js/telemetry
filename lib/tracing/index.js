...
const { certsAvailable } = require('../zti')
  ...
  if (kind === 'telemetry-to-caas') {
    ...
    if (credentials.useZTI) {
      return createLazyExporter(() => {
        ...
        try {
          return { status: 'ok', exporter: new tracingExporterModule[tracingExporter.class](exporterConfig) }
        } catch (error) {
    ...
    const exporter = new tracingExporterModule[tracingExporter.class](exporterConfig)
