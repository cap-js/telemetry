...
function _getSampler() {
  ...
  let _ignoreSpecifiedPaths
  if (!Array.isArray(ignoreIncomingPaths) || !ignoreIncomingPaths.length) _ignoreSpecifiedPaths = () => true
  else {
    // eslint-disable-next-line no-unused-vars
    _ignoreSpecifiedPaths = (context, traceId, name, spanKind, attributes, links) => {
  ...
  function _filterSampler(filterFn, parent) {
    ...
    return {
      shouldSample(context, traceId, name, spanKind, attributes, links) {
        if (!filterFn(context, traceId, name, spanKind, attributes, links)) return { decision: NOT_RECORD }
