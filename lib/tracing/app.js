      ...
      db.prototype.prepare = wrap(prepare, {
        wrapper: function (sql) {
          return trace(`${impl} - prepare ${sql}`, prepare, this, arguments, { loggerName: LOG.label }).then(stmt => {
            const { run, get, all } = stmt
            stmt.run = wrap(run, {
              wrapper: function () {
                return trace(`${impl} - stmt.run ${sql}`, run, this, arguments, { loggerName: LOG.label })
              }
            })
            stmt.get = wrap(get, {
              wrapper: function () {
                return trace(`${impl} - stmt.get ${sql}`, get, this, arguments, { loggerName: LOG.label })
              }
            })
            stmt.all = wrap(all, {
              wrapper: function () {
                return trace(`${impl} - stmt.all ${sql}`, all, this, arguments, { loggerName: LOG.label })
              }
            })
            return stmt
          })
