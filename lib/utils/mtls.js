...
// (ZTI-loaded or statically configured). Both cert and key are required — a mismatched pair
// would only surface as a TLS handshake failure later, so the fail-fast path must reject it.
// Pure predicate: no I/O, no watcher side effects.
function certsAvailable() {
  const x509 = cds.env.requires?.telemetry?.x509
  return !!(x509?.cert && x509?.key)
}
