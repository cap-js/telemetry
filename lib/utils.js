...
function getCredsForCaaSMtls() {
  // Read x509 certificate from cds.env.requires.telemetry
...
function augmentCaaSCreds(credentials) {
  ...
  if (mtlsCreds) {
    ...
    LOG._warn && LOG.warn('CaaS requires mTLS authentication. No mTLS credentials found. Set CDS_REQUIRES_TELEMETRY_X509_CERT and CDS_REQUIRES_TELEMETRY_X509_KEY environment variables (base64 encoded).')
  }
}
