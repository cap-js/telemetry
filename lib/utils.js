...
function getCredsForCLSAsUPS() {
  ...
  // to support connection via user-provided services, the instance must have a tag "cloud-logging"
  ...
  if (vcap['user-provided']?.some(b => b.name.match(/cloud-logging/))) {
    LOG._warn && LOG.warn('User-provided instances of SAP Cloud Logging should have a tag "cloud-logging"')
