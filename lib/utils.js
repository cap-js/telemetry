...
function getCredsForCLSAsUPS() {
  ...
  // to support connection via user-provided services, the instance must have tag "cloud-logging", see: https://cap.cloud.sap/docs/node.js/cds-log#custom-fields
  if (vcap['user-provided']?.some(e => e.tags.includes('cloud-logging')))
    return vcap['user-provided'].find(e => e.tags.includes('cloud-logging')).credentials

  // old implementation, remove or keep for compatibility?
