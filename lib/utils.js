...
const fs = require('fs')
const { initializeZTI, isZTIEnabled, getRotatingAgentFactory, startSVIDWatcher, loadInitialCerts } = require('./zti')
...
function createZTIAgentFactory() {
  ...
  if (!loadInitialCerts()) return null
...
function createStaticAgentFactory() {
  ...
  const { RotatingCertAgent } = require('./zti')
  let agent = null
  return () => {
    if (!agent) agent = new RotatingCertAgent()
    return agent
  }
}
