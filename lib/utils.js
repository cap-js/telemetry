...
const fs = require('fs')
const { getZTICredentials } = require('./zti')
...
// Returns { status: 'ready'|'not_ready'|'disabled', cert?, key? }
function getCredsForCaaSMtls() {
  // Try ZTI first
  const ztiCreds = getZTICredentials()
  if (ztiCreds === null) {
    // ZTI explicitly disabled or no binding
    const { x509 } = cds.env.requires.telemetry || {}
    if (x509 && x509.cert && x509.key) return { status: 'ready', cert: x509.cert, key: x509.key }
    return { status: 'disabled' }
  }

  if (ztiCreds === false) return { status: 'not_ready' } // ZTI configured but SVID files don't exist yet

  return { status: 'ready', cert: ztiCreds.cert, key: ztiCreds.key }
}

function decodeCredentials(mtlsCreds) {
  ...
  return { cert: mtlsCreds.cert, key: mtlsCreds.key }
}
