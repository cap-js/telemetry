...
const fs = require('fs')
const { initializeZTI, isZTIEnabled, getRotatingAgentFactory } = require('./zti')
...
// Returns httpAgentOptions factory for ZTI with automatic certificate rotation
function createZTIAgentFactory() {
  if (!isZTIEnabled()) return null
  if (!initializeZTI()) return null

  return getRotatingAgentFactory()
}
...
// Returns httpAgentOptions factory for static x509 credentials, or null if not configured
function createStaticAgentFactory() {
  const { x509 } = cds.env.requires.telemetry || {}
  if (!x509?.cert || !x509?.key) return null
...
function augmentCaaSCreds(credentials) {
  ...
  LOG._warn && LOG.warn('CaaS mTLS credentials not found')
}

// Creates CaaS exporter with mTLS, using LazyExporter for ZTI deferred initialization
function createCaaSExporter(credentials, config, signalPath, createExporterFn, createLazyExporter, certsAvailable) {
  ...
  // Use LazyExporter if ZTI certs may not be ready yet
  if (credentials.useZTI) {
    return createLazyExporter(() => {
      if (!certsAvailable()) return { status: 'not_ready' }
      try {
        return { status: 'ok', exporter: createExporterFn(exporterConfig) }
      } catch (error) {
  ...
  return createExporterFn(exporterConfig)
}
...
module.exports = {
  ...
  augmentCaaSCreds,
  createCaaSExporter,
  createZTIAgentFactory,
  createStaticAgentFactory,
  decodeCredentials,
  hasDependency,
  _hrnow,
  _require
