...
const fs = require('fs')
const { initializeZTI, isZTIEnabled, getRotatingAgentFactory, loadInitialCerts, certsAvailable } = require('./zti')
...
function wrapExporterWithBuffer(exporter) {
  ...
  exporter.export = function (items, callback) {
    if (!ready) {
      ...
      // Still not ready — buffer
      if (buffer.length >= MAX_BUFFER_SIZE) {
        buffer.shift()
        LOG._warn && LOG.warn('Buffer full, dropping oldest item')
      }
      buffer.push(items)
      callback({ code: 0 })
      return
...
function createCaaSExporter(credentials, config, signalPath, createExporterFn) {
  ...
  if (!certsAvailable() && !credentials.useZTI) {
  ...
  const exporter = createExporterFn(exporterConfig)

  // If ZTI and certs not yet available, wrap with buffering
  if (credentials.useZTI && !certsAvailable()) {
    return wrapExporterWithBuffer(exporter)
  }
