...
function augmentCaaSCreds(credentials) {
  ...
  // Check for certificate-service binding for mTLS
  const certServiceCreds = getCredsForCertService()
  if (certServiceCreds) {
    LOG._info && LOG.info('Found certificate-service binding for CaaS mTLS')
    // Store cert service creds - the app needs to handle certificate enrollment
    credentials._certServiceCreds = certServiceCreds
  } else {
    LOG._warn && LOG.warn('CaaS requires mTLS authentication. No certificate-service binding found.')
    LOG._warn && LOG.warn('Bind a certificate-service instance to enable mTLS.')
  }
}
