...
module.exports = () => {
  ...
  cds.on('connect', function (srv) {
    ...
    srv.after('BEGIN', async function (_, req) {
      ...
      if (!pools.has(cds.context.tenant) && pools.get(undefined) !== pool) {
        cds.context.tenant && LOG._debug && LOG.debug(`Start observing pool for tenant "${cds.context.tenant}"`)
        pools.set(cds.context.tenant, pool)
      }
    })
  })
}
