  ...
  cds.on('listening', () => {
    ...
    cds.db.after(['CREATE'], queueEntity, async (_, req) => {
      ...
      if (!statistics[req.tenant]?.[queuedServiceName]) {
        ...
        const unboxedService = cds.services[queuedServiceName]
        const queuedService = cds.outboxed(unboxedService) // REVISIT: cds^8 compat

        // REVISIT: cds^8 compat
        const maxAttempts = queuedService?.queued?.maxAttempts ?? 20
