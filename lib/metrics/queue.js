  ...
  cds.on('listening', () => {
    ...
    cds.db.after(['CREATE'], queueEntity, async (_, req) => {
      ...
      if (!registeredServics.has(queuedServiceName)) {
        ...
        if (!Object.getOwnPropertySymbols(targetedService).some(symbol => symbol.description === 'queued')) {
          LOG.debug('Skipping registration of queue metrics collection for unqueued service:', queuedServiceName)
          return
        }
