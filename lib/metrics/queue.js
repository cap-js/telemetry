...
function initQueueObservation(statistics) {
  ...
  observables.incomingMessages = meter.createObservableCounter('queue.incoming_messages', {
    description:
      'Number of incoming messages of the queue. Increased by one each time a new message entry is created.',
    unit: 'each',
    valueType: ValueType.INT
  })
...
module.exports = () => {
  ...
  cds.on('listening', () => {
    ...
    cds.db.after(['CREATE'], queueEntity, async (_, req) => {
      ...
      const service = cds.services[queuedServiceName]

      if (!statistics[cds.context.tenant]?.[queuedServiceName]) {
        // Enable statistics collection per queue per tenant
        initTenantQueueStatistics(statistics, cds.context.tenant, queuedServiceName)

        const unqueuedService = cds.unqueued(service)
        const queuedService = cds.queued(service) // REVISIT: cds^8 compat

        // REVISIT: stable access to queue config
        const maxAttempts = queuedService.outboxed.maxAttempts ?? 20

        if (!registeredServics.has(queuedServiceName)) {
          registeredServics.add(queuedServiceName)
          unqueuedService.before('*', () => {
        ...
        if (cds.context.tenant) context.tenant = cds.context.tenant
        ...
        cds.spawn(context, async () => {
          ...
          Object.assign(statistics[context.tenant][queuedServiceName], latestStatistics)
        })
      }

      statistics[cds.context.tenant][queuedServiceName].incomingMessages += 1
    })
  })
}
