  ...
  cds.on('listening', () => {
    ...
    cds.db.after(['CREATE'], queueEntity, async (_, createTaskReq) => {
      ...
      if (!registeredServices.has(queuedServiceName)) {
        ...
        unqueuedService.before('*', queueMetricsCollectionHandler)
        unqueuedService.after('*', () => {
          const tenant = cds.context?.tenant

          // TODO: This is part two of the BAD workaround
          // Reduce the `processing_failures` counter each time ...
          // > the queued service successfully processed a scheduled task
          tenantQueueStastics(statistics, tenant, queuedServiceName).processingFailures -= 1
        })
      }
    })
  })
