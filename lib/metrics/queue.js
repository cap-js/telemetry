...
module.exports = () => {
  ...
  cds.on('listening', () => {
    ...
    cds.db.after(['CREATE'], queueEntity, async (_, req) => {
      ...
      if (!statistics[req.tenant]?.[queuedServiceName]) {
        // Enable statistics collection per queue per tenant
        initTenantQueueStatistics(statistics, req.tenant, queuedServiceName)
      ...
      statistics[req.tenant][queuedServiceName].incomingMessages += 1
    })
  })
}
