    ...
    cds.db.after(["CREATE"], queueEntity, async (_, req) => {
      ...
      if (!statistics[req.tenant]?.[outboxedServiceName]) {
        ...
        if (!registeredServics.has(outboxedServiceName)) {
          ...
          unboxedService.before("*", (r) => {
            statistics[r.tenant][outboxedServiceName].outgoingMessages += 1;
          })
        }
        ...
        context.every =
          cds.env.requires.telemetry.metrics.config.exportIntervalMillis / 2;
        ...
        cds.spawn(context, async () => {
          ...
          Object.assign(
            statistics[context.tenant][outboxedServiceName],
            latestStatistics
          );
        });
      }
