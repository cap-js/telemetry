...
function initOutboxObservation(statistics) {
  ...
  observables.remainingEntries = meter.createObservableGauge('outbox.remaining_entries', {
    description: 'Number of entries which are pending for delivery.',
    unit: 'each',
    valueType: ValueType.INT
  })

  observables.minStorageTimeSeconds = meter.createObservableGauge('outbox.min_storage_time_in_seconds', {
    description: 'Minimal time in seconds an entry was stored in the outbox.',
    unit: 'seconds',
    valueType: ValueType.INT
  })

  observables.medStorageTimeSeconds = meter.createObservableGauge('outbox.med_storage_time_in_seconds', {
    description: 'Median time in seconds of an entry stored in the outbox.',
    unit: 'seconds',
    valueType: ValueType.INT
  })

  observables.maxStorageTimeInSeconds = meter.createObservableGauge('outbox.max_storage_time_in_seconds', {
    description: 'Maximum time in seconds an entry was residing in the outbox.',
    unit: 'seconds',
    valueType: ValueType.INT
  })

  observables.incomingMessages = meter.createObservableCounter('outbox.incoming_messages', {
    description:
  ...
  observables.outgoingMessages = meter.createObservableCounter('outbox.outgoing_messages', {
    description: 'Number of outgoing messages of the outbox. Increased by one each time a delivery attempt is made.',
    unit: 'each',
    valueType: ValueType.INT
  })
  ...
  cds.on('listening', () => {
    ...
    cds.db.after(['CREATE'], queueEntity, async (_, req) => {
      ...
      if (!statistics[req.tenant]?.[outboxedServiceName]) {
        ...
        if (!registeredServics.has(outboxedServiceName)) {
          registeredServics.add(outboxedServiceName)
        ...
        if (req.tenant) context.tenant = req.tenant
        context.every = cds.env.requires.telemetry.metrics.config.exportIntervalMillis / 2

        cds.spawn(context, async () => {
          const latestStatistics = await collectLatestOutboxInfo(queueEntity, outboxedServiceName, maxAttempts)

          Object.assign(statistics[context.tenant][outboxedServiceName], latestStatistics)
        })
      }
