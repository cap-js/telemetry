...
const { getDynatraceMetadata, getCredsForDTAsUPS, getCredsForCLSAsUPS, augmentCLCreds, augmentCaaSCreds, getCredsForCaaSMtls, _require } = require('../utils')
...
function _getExporter() {
  ...
  if (kind.match(/to-caas$/)) {
    ...
    const lazyExporter = createLazyExporter(() => {
      ...
      return new metricsExporterModule[metricsExporter.class](exporterConfig)
    })
    LOG._debug && LOG.debug('Using lazy metrics exporter for CaaS')
    return lazyExporter
  }
