  ...
  if (kind === 'telemetry-to-caas') {
    ...
    if (credentials.useZTI) {
      return createLazyExporter(() => {
        ...
        try {
          return { status: 'ok', exporter: new metricsExporterModule[metricsExporter.class](exporterConfig) }
        } catch (error) {
    ...
    const exporter = new metricsExporterModule[metricsExporter.class](exporterConfig)
