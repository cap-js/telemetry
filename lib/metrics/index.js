...
function _getExporter() {
  ...
  if (kind.match(/to-caas$/)) {
    ...
    config.url ??= credentials.url
    // Pass mTLS agent options if available (OTLP exporter uses 'agentOptions')
    if (credentials.httpAgentOptions) {
      config.agentOptions = credentials.httpAgentOptions
      LOG._info && LOG.info('CaaS metrics exporter config: url=' + config.url + ', agentOptions.cert exists=' + !!config.agentOptions.cert)
    } else {
