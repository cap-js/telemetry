...
function _getExporter() {
  ...
  if (dynatrace) {
    ...
    const token = dynatrace.tokens?.find(token => token.scopes?.includes('metrics.ingest')) || dynatrace.metrics_apitoken || dynatrace.rest_apitoken?.token
    if (!token) throw new Error('No Dynatrace API token found with scope "metrics.ingest"')
