...
module.exports = class OTELInitializer {
  ...
  static registerProvider() {
    ...
    if (!cds.env.requires.otel.trace.tracer) cds.env.requires.otel.trace.tracer = trace.getTracer(cds.env.requires.otel.trace.name, cds.env.requires.otel.trace.version)
    addInstrumentation()
        ...
        switch (propa) {
          ...
          case 'jaeger':
            if (dependencyExists('@opentelemetry/propagator-jaeger')) {
              ...
              LOG.error(`please run 'npm i @opentelemetry/propagator-jaeger' to use jaeger propagator else it will not work`)
            }
            break
          default:
            propagators.push(new W3CTraceContextPropagator())
  ...
  static getExporters() {
      ...
      try {
        ...
        if (cds.env.requires.otel.trace.export === 'jaeger') {
          if (dependencyExists('@opentelemetry/exporter-jaeger')) {
            ...
            LOG.error(`Please run 'npm i @opentelemetry/exporter-jaeger' to be able to use jaeger as an exporter for the OpenTelemetry plugin`)
          }
    ...
    if (!cds.env.requires.otel.metrics.exporter) {
      try {
        if (!cds.env.requires.otel.metrics.export) cds.env.requires.otel.metrics.export = cds.env.requires.otel.trace.export
        ...
        if (cds.env.requires.otel.metrics.export === 'grpc') {
          if (dependencyExists('@opentelemetry/exporter-trace-otlp-grpc')) {
            ...
            cds.env.requires.otel.metrics.exporter = new OTLPMetricExporterGrpc(cds.env.requires.otel.metrics.exportOptions)
          } else {
          ...
          cds.env.requires.otel.metrics.exporter = new OTLPMetricExporterProto(cds.env.requires.otel.metrics.exportOptions)
          ...
          if (cds.env.requires.otel.metrics.format === 'json')
            cds.env.requires.otel.metrics.exporter = new ConsoleMetricExporter(cds.env.requires.otel.metrics.exportOptions)
          else cds.env.requires.otel.metrics.exporter = new CDSConsoleMetricsExporter(cds.env.requires.otel.metrics.exportOptions)
        }
      } catch (error) {
        throw new Error(`Error during initialization of Exporter , ${error}`)
      }
...
function ignoreSpecifiedPaths(spanName, spanKind, attributes) {
  return (
    ...
    (Array.isArray(cds.env.requires.otel.trace.ignorePaths) && !cds.env.requires.otel.trace.ignorePaths.some(path => path === spanName)
